...
def handle_content_message(event):
    ...
    renamelist['ชื่อโรค'] = renamelist['ชื่อโรค'].replace(["powder","n_loss","mg_loss","blight","spot"],["โรคราแป้ง","ขาดธาตุ N","ขาดธาตุ Mg","โรคใบไหม้","โรคใบจุด"])
    ...
    if df.empty:
        line_bot_api.reply_message(
            event.reply_token, [
                ...
                #TextSendMessage(text=f'ผลจากการวิเคราะห์ : {name}\nความแม่นยำ : {r.confidence:.2f}\n\n-- สรุปผลการวิเคราะห์ --\n{renamelist}\n\n-- กรุณาระบุชนิดของโรคที่เมนูด้านล่าง --'),
                TextSendMessage(text=f'ไม่สามารถวิเคราะห์รูปภาพได้ กรุณาลองใหม่อีกครั้ง'),
            ])
    else :
        line_bot_api.reply_message(
            event.reply_token, [
                ImageSendMessage(url,url),
                #TextSendMessage(text=f'ผลจากการวิเคราะห์ : {name}\nความแม่นยำ : {r.confidence:.2f}\n\n-- สรุปผลการวิเคราะห์ --\n{renamelist}\n\n-- กรุณาระบุชนิดของโรคที่เมนูด้านล่าง --'),
                TextSendMessage(text=f'-- สรุปผลการวิเคราะห์ --\n{renamelist}\n\n-- กรุณาระบุชนิดของโรคที่เมนูคุยกับน้องเรียน --'),
            ])
